...
This tool is designed to download anime with desired quality and translation from anime365.ru website.

## Naming conventions & coding standards

In the crates directory where a365dt Rust code lives:

- All crates sit under crates subdirectory.
- Crate names start with `a365dt-`. For example, the `core` folder crate should be named `a365dt-core`.
...
2. Run the test for the specific project that was changed. For example, if changes were made in `crates/a365dt-cli`, run `just test -p a365dt-cli`.
...
4. Never run release profile to check and test changes. Always use `a365dt-dev`.
